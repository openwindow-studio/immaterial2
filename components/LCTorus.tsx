...
function TorusMesh({ size = 1, enableOrbitControls = true }: { size?: number; enableOrbitControls?: boolean }) {
  const mesh = useRef<any>()
  const backgroundGroup = useRef<any>()
  ...
  const uniforms = useMemo(() => ({
    ...
    uRefractPower: { value: hardcodedParams.refraction },
    ...
    uLight: { value: hardcodedParams.light },
    winResolution: {
  ...
  return (
    <>
      <group ref={backgroundGroup} visible={false}>
        <mesh position={[-1.44 * size, -1.008 * size, -1.44 * size]}>
          <icosahedronGeometry args={[0.72 * size, 16]} />
        ...
        <mesh position={[-1.728 * size, 1.008 * size, -1.44 * size]}>
          <icosahedronGeometry args={[0.72 * size, 16]} />
      ...
      <mesh ref={mesh}>
        <torusGeometry args={[3.456 * size, 1.296 * size, 32, 100]} />
        <shaderMaterial
          key={uuidv4()}
...
export default function LCTorus({
  size = 1,
  background = 'transparent',
  enableOrbitControls = true,
  style = {},
  canvasStyle = {},
  cameraPosition = [4, -2, 7] as [number, number, number]
}) {
  return (
    <div style={{
      position: 'absolute',
      ...
      <Canvas
        ...
        style={{ width: '100%', height: '100%', background: 'transparent', ...canvasStyle }}
      >
        <ambientLight intensity={1.0} />
        <TorusMesh size={size} enableOrbitControls={enableOrbitControls} />
      </Canvas>
    </div>
  )
}